...
%%
%% 1) the module name is gen_server_rabbit
%%
%% 2) more efficient handling of selective receives in callbacks
%% gen_server_rabbit processes drain their message queue into an internal
...
%% queue of a gen_server_rabbit process is the concatenation of the internal
...
%%
%% 3) gen_server_rabbit:cast is guaranteed to be order-preserving
...
%%
%% The purpose is that the gen_server_rabbit takes care of adjusting the
...
%% you need to use gen_server_rabbit:reply/2 to issue the reply manually.
%%
%% 8) The callback module can optionally implement
%% format_message_queue/2 which is the equivalent of format_status/2
%% but where the second argument is specifically the priority_queue_rabbit
...
enter_loop(Mod, Options, State, ServerName, Timeout, Backoff) ->
    ...
    Queue = priority_queue_rabbit:new(),
...
process_next_msg(GS2State = #gs2_state { time          = Time,
                                         ...
                                         queue         = Queue }) ->
    case priority_queue_rabbit:out(Queue) of
        {{value, Msg}, Queue1} ->
...
in(Input, Priority, GS2State = #gs2_state { queue = Queue }) ->
    GS2State # gs2_state { queue = priority_queue_rabbit:in(Input, Priority, Queue) }.
...
format_status(Opt, StatusData) ->
    ...
    Messages = callback(Mod, format_message_queue, [Opt, Queue],
                        fun () -> priority_queue_rabbit:to_list(Queue) end),
    [{header, Header},
